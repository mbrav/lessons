...
# Master 2
resource "libvirt_cloudinit_disk" "debian_cloud_master_2" {
  name = "debian_cloud_master_2.iso"
...
resource "libvirt_volume" "kubernetes_master_2_disk" {
...
resource "libvirt_domain" "kubernetes_master_2" {
  name   = "kubernetes_master_2"
  memory = "3072"
  vcpu   = 2

  # Mount cloud-init image
  cloudinit = libvirt_cloudinit_disk.debian_cloud_master_2.id
  ...
  disk {
    volume_id = libvirt_volume.kubernetes_master_2_disk.id
  }
