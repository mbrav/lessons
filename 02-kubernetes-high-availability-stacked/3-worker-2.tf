...
# Worker 2
resource "libvirt_cloudinit_disk" "debian_cloud_worker_2" {
  name = "debian_cloud_worker_2.iso"
...
resource "libvirt_volume" "kubernetes_worker_2_disk" {
...
resource "libvirt_domain" "kubernetes_worker_2" {
  name   = "kubernetes_worker_2"
  memory = "3072"
  vcpu   = 4

  # Mount cloud-init image
  cloudinit = libvirt_cloudinit_disk.debian_cloud_worker_2.id
  ...
  disk {
    volume_id = libvirt_volume.kubernetes_worker_2_disk.id
  }
