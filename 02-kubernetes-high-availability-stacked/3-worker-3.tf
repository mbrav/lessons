...
# Worker 3
resource "libvirt_cloudinit_disk" "debian_cloud_worker_3" {
  name = "debian_cloud_worker_3.iso"
...
resource "libvirt_volume" "kubernetes_worker_3_disk" {
...
resource "libvirt_domain" "kubernetes_worker_3" {
  name   = "kubernetes_worker_3"
  memory = "3072"
  vcpu   = 4

  # Mount cloud-init image
  cloudinit = libvirt_cloudinit_disk.debian_cloud_worker_3.id
  ...
  disk {
    volume_id = libvirt_volume.kubernetes_worker_3_disk.id
  }
